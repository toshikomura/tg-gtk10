...
adaptação de uma gema adicionando novas funcionalidade que geralmente utilizamos. 
...
tomar, e fechando o capítulo com a seção \ref{section:exemplo_de_uso_de_google-maps-for-rails}, onde
apresentaramos o uso da gema de exemplo que adaptamos. Caso haja interesse, pode-se consultar o apêndice
\ref{chapter:api_do_google_maps} para conhecer um pouco da história do \emph{Google Maps} e sua \emph{API}.
...
os diagrama de classe na imagem \ref{fig:diagrama_de_classes_google_maps_for_rails}, o diagrama de
...
Para as imagens de diagrama de classes \ref{fig:diagrama_de_classes_google_maps_for_rails} e o diagrama
de atributos na imagem \ref{fig:diagrama_de_atributos_google_maps_for_rails} as seguintes explicações
são válidas:
...
As seguintes explicações são válidas para o diagrama de de atributos na imagem
...
Agora que realizamos a \emph{engenharia reversa} da \emph{gema}, podemos analisar algumas
características dos diagramas e responder algumas perguntas como por exemplo.
...
Na adaptação de exemplo, somente inserimos 4 classes e 2 funções e com isso podemos perceber que as modificações que
fizemos não afetaram o funcionamento da gema, pois não foi necessário mexer no código que já existia.
...
Em seguida para fazer o uso da função de direções utilizamos o código
\ref{lst:exemplo_coffeescript_que_cria_mapa_com_direcao} explicado logo abaixo.
...
\ref{fig:caminho_entre_sao_paulo_e_curitiba}. Neste caso a
nossa gema adaptada com a nova funcionalidade de direções funcionou corretamente, pois o caminho
mostrado é entre ‘‘São Paulo'' e ‘‘Curitiba'' como requisitamos na linha ‘‘24'' do código
\ref{lst:exemplo_coffeescript_que_cria_mapa_com_direcao}.
